...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **492** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 492 автотеста, ~7 секунд
```

- **492 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
├── tests/                      # 492 автотеста (юнит + хендлеры)
