...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **358** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 358 автотестов, ~7 секунд
```

- **358 автотестов** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 36 ачивок, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- **⚔️ Босс-файт за 1-е место** — место #1 занимается не по очкам, только победой над текущим чемпионом в отдельном матче (×2 к рейтингу, ничья запрещена, блокировка мгновенного реванша)
- Система достижений (36 ачивок, 17 из них скрыты до разблокировки, сгруппированы по категориям) и пасхалки после матчей
...
├── tests/                      # 358 автотестов (юнит + хендлеры)
