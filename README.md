...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **300** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 300 автотеста, ~7 секунд
```

- **300 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 31 ачивку, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- Система достижений (31 ачивка, 12 из них скрыты до разблокировки) и пасхалки после матчей
...
- **Рекорды клуба** — все рекорды за всё время: серии (лучшая и текущая активная), высший рейтинг в истории, дольше всех лидировал на 1-м месте, длинные и самые быстрые матчи, апсеты, эпичные матчи, дерби клуба (самая играющая пара), нагибатор клуба (самое одностороннее противостояние), больше всего ничьих, самый жаркий день клуба
...
├── tests/                      # 300 автотеста (юнит + хендлеры)
