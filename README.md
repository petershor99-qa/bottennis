...
```bash
pytest -v          # 310 автотестов, ~7 секунд
```

- **310 автотестов** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
├── tests/                      # 310 автотестов (юнит + хендлеры)
