...
<p align="center">
  <img src="docs/screenshots/menu.png" width="19%" alt="Главное меню" />
  <img src="docs/screenshots/leaderboard.png" width="19%" alt="Рейтинг игроков" />
  <img src="docs/screenshots/match.png" width="19%" alt="Экран матча" />
  <img src="docs/screenshots/weekly.png" width="19%" alt="Итоги недели" />
  <img src="docs/screenshots/records.png" width="19%" alt="Рекорды клуба" />
</p>
...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **209** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 209 автотеста, ~5 секунд
```

- **209 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 28 ачивок, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- Система достижений (28 ачивок) и пасхалки после матчей
...
- **Рекорды клуба** — все рекорды за всё время: серии (лучшая и текущая активная), высший рейтинг в истории, длинные матчи, апсеты, эпичные матчи, дерби клуба (самая играющая пара), нагибатор клуба (самое одностороннее противостояние)
...
├── tests/                      # 209 автотеста (юнит + хендлеры)
