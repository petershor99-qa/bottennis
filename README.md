...
```bash
pytest -v          # 735 автотеста, ~7 секунд
```

- **735 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
├── tests/                      # 735 автотеста (юнит + хендлеры)
