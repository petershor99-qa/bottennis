...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **646** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 646 автотеста, ~7 секунд
```

- **646 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
- **Рекорды клуба** — все рекорды за всё время: серии (лучшая и текущая активная), высший рейтинг в истории, дольше всех лидировал на 1-м месте, самое короткое правление, больше всего восхождений на трон, больше всего защит трона подряд, длинные и самые быстрые матчи, самый долгий боссфайт, апсеты, эпичные матчи, дерби клуба (самая играющая пара), нагибатор клуба (самое одностороннее противостояние), долгожданная месть, железные нервы (наименьший разброс рейтинга за карьеру), больше всего ничьих, самый жаркий день клуба
...
├── tests/                      # 646 автотеста (юнит + хендлеры)
