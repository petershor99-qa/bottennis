...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **503** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 503 автотеста, ~7 секунд
```

- **503 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
├── tests/                      # 503 автотеста (юнит + хендлеры)
