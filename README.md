...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **696** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 696 автотеста, ~7 секунд
```

- **696 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 58 ачивок, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- 🏅 **58 достижений** (27 скрытых) + **7 личных рекордов** + пасхалки после матчей
...
- Система достижений (58 ачивок, 27 из них скрыты до разблокировки, сгруппированы по категориям) и пасхалки после матчей
...
### Автосводки и прочее
- 📅 Итоги дня в 21:30 МСК — топ дня с полоской формы, «матч дня», нагибатель дня, «дуэль дня» (самая частая пара + счёт между ними), лучший и отрицательный рост, «американские горки» (волатильность рейтинга), «без поражений», суммарные очки
...
├── tests/                      # 696 автотеста (юнит + хендлеры)
