...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **142** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 147 автотестов, ~5 секунд
```

- **147 автотестов** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
- 📊 **График динамики рейтинга** — линия рейтинга по матчам через quickchart.io
- **Рекорды клуба** — все рекорды за всё время: серии, длинные матчи, апсеты, эпичные матчи
...
├── tests/                      # 147 автотестов (юнит + хендлеры)
