...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **363** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 363 автотестов, ~7 секунд
```

- **363 автотестов** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 37 ачивок, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- Система достижений (37 ачивок, 18 из них скрыты до разблокировки, сгруппированы по категориям) и пасхалки после матчей
...
├── tests/                      # 363 автотестов (юнит + хендлеры)
