...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **472** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 472 автотеста, ~7 секунд
```

- **472 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 43 ачивки, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- **⚔️ Босс-файт за 1-е место** — место #1 занимается не по очкам, только победой над текущим чемпионом в отдельном матче (×2 к рейтингу, ничья запрещена, блокировка мгновенного реванша). Если статус претендента пропадает раньше боссфайта (чемпион обогнал обратно, сам проиграл, или его тихо обогнал кто-то третий) — «💸 ПОТРАЧЕНО» и скрытая ачивка «Просран шанс»
- Система достижений (43 ачивки, 24 из них скрыты до разблокировки, сгруппированы по категориям) и пасхалки после матчей
...
- **Рекорды клуба** — все рекорды за всё время: серии (лучшая и текущая активная), высший рейтинг в истории, дольше всех лидировал на 1-м месте, больше всего защит трона подряд, длинные и самые быстрые матчи, самый долгий боссфайт, апсеты, эпичные матчи, дерби клуба (самая играющая пара), нагибатор клуба (самое одностороннее противостояние), больше всего ничьих, самый жаркий день клуба
...
- **Расширенная статистика/профиль** — тренд за 30 дней, партии на дьюсе, разрыв до соседа по таблице, дистанция до трона (боссфайт)
- **Экран «С кем сыграть?»** — активные матчи клуба + рекомендации соперника с сигналом (серия поражений, реванш, пауза, сильный соперник)
- **Экран «Сегодня»** — счёт дня по всем игрокам + личный мини-итог зрителя (его счёт и дельта за день)
...
- 📅 Итоги дня в 21:30 МСК — топ дня с полоской формы, «матч дня», нагибатель дня, дерби дня, лучший и отрицательный рост, «американские горки» (волатильность рейтинга), «без поражений», суммарные очки
- 📊 Еженедельный дайджест (личный прогресс до ачивки, «халявщик недели») + **итоги месяца** 1-го числа в 10:00 МСК + **итоги квартала** 1-го числа января/апреля/июля/октября в 10:30 МСК (самая насыщенная сводка — топ, рост, серии, «без поражений», «американские горки», топ-матч)
...
- Итоги года 31 декабря в 14:00 МСК, индекс формы (рейтинг по последним 10 матчам), расширенный репортаж топ-матча периода
- **Звания по рейтингу** (Джун/Миддл/Сеньор/Тим лид/Ген дир), **зал славы** (история правлений на месте #1), **карточка «поделиться победой»**, постоянная клавиатура быстрого доступа (Вызвать на матч / Рейтинг / Статистика)
...
│   │   └── admin.py            # админ-команды (/dbstats, /backup)
│   ├── services/
...
├── tests/                      # 472 автотеста (юнит + хендлеры)
