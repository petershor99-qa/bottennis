...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **686** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 686 автотеста, ~7 секунд
```

- **686 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 57 ачивок, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- 🏅 **52 достижения** (24 скрытых) + **7 личных рекордов** + пасхалки после матчей
...
- **Радар личного стиля** — кнопка «Стиль» на «Статистике», 4 оси (винрейт, % на дьюсе, камбэки, стабильность) через quickchart
...
- Система достижений (57 ачивок, 26 из них скрыты до разблокировки, сгруппированы по категориям) и пасхалки после матчей
...
- **Рекорды клуба** — все рекорды за всё время: серии (лучшая и текущая активная), высший рейтинг в истории, дольше всех лидировал на 1-м месте, самое короткое правление, больше всего восхождений на трон, больше всего защит трона подряд, длинные и самые быстрые матчи, самый долгий боссфайт, апсеты, эпичные матчи, дерби клуба (самая играющая пара), нагибатор клуба (самое одностороннее противостояние), равный бой (самое сбалансированное противостояние), долгожданная месть, железные нервы (наименьший разброс рейтинга за карьеру), больше всего ничьих, самый жаркий день клуба
...
- 📊 Еженедельный и ежемесячный дайджесты — личная шапка (счёт, рейтинг, прогресс до ачивки) + общий клубный блок с той же полоской формы, «халявщиком», «без поражений», «американскими горками», что и в итогах дня
- **Итоги квартала** (1-го числа января/апреля/июля/октября, 10:30 МСК) — самая насыщенная периодическая сводка
- **Итоги года** (31 декабря, 14:00 МСК) — полный табель клуба, чемпион года, взлёт/падение, дерби, топ-матч года
...
- Постоянная клавиатура быстрого доступа снизу (Вызвать на матч / Рейтинг / Статистика)
...
├── tests/                      # 686 автотеста (юнит + хендлеры)
