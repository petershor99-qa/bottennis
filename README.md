...
```bash
pytest -v          # 248 автотеста, ~5 секунд
```

- **248 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
- Система достижений (30 ачивок, 11 из них скрыты до разблокировки) и пасхалки после матчей
...
├── tests/                      # 248 автотеста (юнит + хендлеры)
