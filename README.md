...
Это **не учебный todo-лист, а живой бот в проде**: я спроектировал, написал и **покрыл тестами** весь цикл — от регистрации по инвайту до пересчёта рейтинга и вечерних сводок. Каждую фичу сопровождал автотестами — сейчас их **729** (включая тесты хендлеров на сам флоу), плюс CI на каждый push и привычка проверять граничные случаи. Регрессии отлавливаются раньше, чем долетают до игроков — и быстрее, чем я успеваю проиграть очередную партию.
...
```bash
pytest -v          # 729 автотеста, ~7 секунд
```

- **729 автотеста** на `pytest` + `pytest-asyncio` (юнит чистой логики + тесты хендлеров)
...
| Достижения | `tests/test_achievements.py` | все 59 ачивок, идемпотентность, бэкфилл по истории (in-memory SQLite) |
...
- 🏅 **59 достижений** (27 скрытых) + **7 личных рекордов** + пасхалки после матчей
...
- Система достижений (59 ачивок, 27 из них скрыты до разблокировки, сгруппированы по категориям) и пасхалки после матчей
...
├── tests/                      # 729 автотеста (юнит + хендлеры)
