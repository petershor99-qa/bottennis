...
COMEBACK_OPENERS = [
    ...
    "{name} был в шаге от поражения при 0:2 — прямо как в финале «Бригады», всё висело на волоске. ",
    "Казалось, {name} уже досрочно проиграл при 0:2 — фильм должен был закончиться на 20-й минуте, но нет. ",
    "{name} отыгрался с 0:2 — и это многое говорит о характере. ",
]
...
MARATHON_FRAGMENTS = [
    ...
    "матч дошёл до пятой партии — тут уже включается характер, а не техника",
    "растянули удовольствие на все 5 партий",
    "дошли до решающей — оба явно не хотели уступать",
    "пятая партия — редкость, но тут случилась",
]
...
DEUCE_FRAGMENTS = [
    ...
    "а решающая партия дожималась на дьюсе — саспенс на уровне Хичкока",
    "решающая партия — дьюс на надрыв всего",
    "а концовка решающей — как в хорошем триллере, до последней секунды непонятно",
    "решающая партия ушла в дьюс и там осталась надолго",
]
...
UPSET_FRAGMENT_TEMPLATES = [
    ...
    "и это ещё +{delta} pts апсета — Давид против Голиафа в чистом виде",
    "а по рейтингу это +{delta} pts настоящего апсета",
    "плюс {delta} pts, которых от него не ждали",
...
def _stable_pool_index(seed: int, salt: str, pool_len: int) -> int:
    ...
    Линейная функция вида (seed*k + c) % n при n одинаковом для нескольких
    пулов даёт индекс, полностью определяемый seed mod n, — какой множитель
    k ни возьми, комбинация индексов НЕСКОЛЬКИХ пулов одинакового размера
    физически не может иметь больше n вариантов (математическое свойство
    линейных функций по модулю, не баг реализации). Наткнулись на это, когда
    все 4 пула match_report() выросли до одинакового размера — регресс-тест
    на декорреляцию пулов сразу поймал совпадение.

    MD5, не zlib.crc32 — CRC32 линейна над GF(2) и НЕ даёт лавинного эффекта
    на похожих коротких строках ("4:comeback" и "9:comeback" давали заметно
    коррелирующие остатки по модулю 8 — проверено вручную, разные mid давали
    одинаковую комбинацию индексов у всех 4 факторов сразу). MD5 — настоящий
    криптографический хэш с честным распределением, даже на смежных seed.
    """
...
def match_report(m: Match, winner_name: str) -> str:
    ...
    через `_stable_pool_index(m.id, соль, len(пула))` (v2.119.0): простое
    линейное смещение (+1/+2/+3) НЕ декоррелирует пулы одинакового размера —
    комбинация индексов физически не может иметь больше вариантов, чем сам
    размер пула, при любом множителе (см. докстринг `_stable_pool_index`).
    Хэш с разной солью на каждый пул разрывает эту связь.
    ...
    if comeback:
        ...
        opener = COMEBACK_OPENERS[idx].format(name=h(winner_name))

    tail_fragments = []
    if marathon:
        idx = _stable_pool_index(m.id, "marathon", len(MARATHON_FRAGMENTS))
        tail_fragments.append(MARATHON_FRAGMENTS[idx])
    if deuce_decider:
        idx = _stable_pool_index(m.id, "deuce", len(DEUCE_FRAGMENTS))
        tail_fragments.append(DEUCE_FRAGMENTS[idx])
    if upset:
...
H2H_DRAW_PHRASES = [
    ...
    "Тогда никто не дожал. Сегодня дожали",
    "Санта-Барбара продолжается — но на этот раз с победителем",
    "После той ничьей давно пора было выяснить, кто сильнее",
    "В прошлый раз разошлись миром. Сегодня без вариантов",
]
...
def random_challenge_button_label() -> str:
    """Подпись кнопки «Вызвать на матч» — ОДИНАКОВАЯ одновременно в постоянной
    клавиатуре снизу и в инлайн-меню (v2.119.0, фикс живой жалобы пользователя).

    Раньше был чистый random.choice() — два независимых вызова (один при
    /start для постоянной клавиатуры, другой при КАЖДОЙ отрисовке инлайн-меню)
    почти всегда расходились текстом, хотя обе кнопки видны на экране в одно
    и то же время. Индекс завязан на текущий час по UTC вместо случайного
    выбора — детерминированная функция времени, а не состояние: любой вызов
    в рамках одного часа (независимо от места и повода) возвращает один и тот
    же вариант. Подпись всё ещё меняется со временем (раз в час, не на каждый
    рендер) — тот же эффект «не примелькалось», но без рассинхрона."""
    hour_bucket = int(datetime.now(timezone.utc).timestamp() // 3600)
    return CHALLENGE_BUTTON_LABELS[hour_bucket % len(CHALLENGE_BUTTON_LABELS)]
