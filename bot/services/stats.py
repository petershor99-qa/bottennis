...
#
# Порог «Дожимание» поднят с 70 до 85 (v2.130.0, живая жалоба пользователя —
# «Финишер» доставался почти всем): в формате до 2 побед (Bo3/Bo5) выигрыш
# первой партии сам по себе уже сильно фаворитит игрока чисто математически
# (нужно довыиграть меньше партий, чем сопернику) — конверсия 70-80% типична
# почти для любого игрока, а не признак особого умения дожимать. 85%+ —
# уже заметно выше этой структурной базы.
#
# 4 архетипа-«лузера» (v2.130.0, прямая просьба пользователя) — зеркало
# положительных архетипов на низкие значения тех же 4 осей, тот же дух, что
# у loser-ачивок v2.127.0 («Слил 2:0», «Долина слёз» и т.п.). «Доминирование»
# зеркала не получило — узкая победа это не поражение, для лузерского
# архетипа не годится. «Стабильность» уже зеркалена «Американскими горками».
_ARCHETYPE_RULES: list[tuple[str, float, bool, str]] = [
    ("Винрейт", 60.0, False, "Терминатор"),
    ("Клатч", 60.0, False, "Нервы стальные"),
    ("Дожимание", 85.0, False, "Финишер"),
    ...
    ("Дожимание", 30.0, True, "Сдувается"),
    ("Камбэки", 1.0, True, "Точка невозврата"),
]
...
# пользователя: сам ярлык («Финишер», «Каток») ничего не объяснял без
# контекста, приходилось догадываться. Ключи — те же названия, что в
# _ARCHETYPE_RULES выше.
_ARCHETYPE_DESCRIPTIONS: dict[str, str] = {
    "Терминатор": "выигрываешь чаще, чем проигрываешь, и с солидным отрывом",
    "Нервы стальные": "на дьюсе почти не дрогнешь — берёшь верх в решающие моменты",
    "Финишер": "взял первую партию — матч почти всегда за тобой",
    "Каток": "побеждаешь уверенно, с большим перевесом по очкам",
    "Феникс": "умеешь отыгрываться с 0:2 — характер решает исход",
    "Метроном": "рейтинг двигается ровно, без резких качелей",
    "🎢 Американские горки": "рейтинг штормит — взлёты и падения через игру",
    "Донор рейтинга": "проигрываешь чаще, чем выигрываешь",
    "Мандраж": "теряешься именно на дьюсе — решающие очки не твоя сильная сторона",
    "Сдувается": "взял первую партию — а матч почти всегда упускаешь",
    "Точка невозврата": "если ведут 0:2, для тебя на этом всё — камбэков не бывает",
}


def _archetype_description(archetype: str) -> str | None:
    """Короткая расшифровка архетипа для подписи под радаром, см. выше."""
    return _ARCHETYPE_DESCRIPTIONS.get(archetype)


# Короткая расшифровка каждой оси радара (v2.130.0) — та же жалоба
# пользователя, что и у архетипов: «Клатч 56%» ничего не говорит без
# пояснения, что это вообще такое. Порядок ключей совпадает с порядком осей
# в _build_style_radar — используется для построения подписи в том же порядке,
# что и на самом графике.
AXIS_GLOSSARY: dict[str, str] = {
    "Винрейт": "доля побед",
    "Клатч": "винрейт на дьюсе",
    "Дожимание": "довёл матч после победы в 1-й партии",
    "Камбэки": "победы, начатые с 0:2",
    "Доминирование": "перевес по очкам в победах",
    "Стабильность": "ровность рейтинга",
}
